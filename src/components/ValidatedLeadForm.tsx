    ...
    <form onSubmit={onSubmit} noValidate className={compact ? 'grid gap-3.5' : 'grid gap-5'} aria-live="polite">
          ...
          <div className={compact ? '' : 'sm:col-span-2'}>
            <div className="space-y-1.5">
              ...
              <select
                ...
                className={`w-full rounded-xl border bg-white px-4 text-sm text-slate-900 transition-all duration-300 focus:outline-none focus:ring-2 ${
                ...
                } ${submitting ? 'cursor-not-allowed opacity-70' : 'hover:border-slate-300'} ${compact ? 'py-2.5' : 'py-3'}`}
                disabled={submitting}
              ...
              {showError('projectType') && errors.projectType && (
                <p className="text-xs font-medium text-red-600 animate-in fade-in duration-200">{errors.projectType}</p>
              )}
            </div>
          </div>
        )}
          ...
          <div className={compact ? '' : 'sm:col-span-2'}>
            <div className="space-y-1.5">
              ...
              {showError('service') && errors.service && (
                <p className="text-xs font-medium text-red-600 animate-in fade-in duration-200">{errors.service}</p>
              )}
            </div>
          </div>
        )}
      ...
      <div className="space-y-2.5">
        ...
        >
          <span>
            ...
            <a href={privacyHref} className="text-cyan-600 hover:text-cyan-700"> {strings.privacyPolicy} </a>
            {` ${strings.consentJoiner} `}
            <a href={termsHref} className="text-cyan-600 hover:text-cyan-700">{strings.termsAndConditions}</a>
          </span>
        </ConsentCheckbox>
        {showError('requiredConsent') && <p className="text-xs text-red-600">{errors.requiredConsent}</p>}
      ...
      <button
        ...
        disabled={!isValid || submitting}
          className={`rounded-full bg-blue-600 px-6 py-3 font-bold text-white shadow-[0_0_18px_rgba(37,99,235,0.35)] transition-all duration-300 ${
      ...
      {submitted && <p className="text-sm font-semibold text-cyan-600">{strings.submitSuccess}</p>}
