      ...
      <div className="container mx-auto px-6 opacity-0 translate-y-4" data-reveal data-stagger-parent>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:mt-10 md:grid-cols-3">
          {items.map((item) => (
            <article
              key={item.title}
              className="group rounded-2xl border border-slate-200/90 bg-white p-6 backdrop-blur-md opacity-0 translate-y-3 transition-all duration-500 ease-out hover:-translate-y-1 hover:border-cyan-400/65 hover:shadow-[0_14px_30px_rgba(15,23,42,0.45)] hover:shadow-cyan-400/20"
              data-stagger-item
            >
              <div className="mb-4 flex items-center justify-between gap-3">
                <div className="inline-flex rounded-xl border border-slate-200 bg-white p-3 text-cyan-700 transition-all duration-300 group-hover:border-cyan-400/70">
