  ...
  return (
    <section className="bg-slate-50 py-16 md:py-20">
      <div className="container mx-auto px-6 opacity-0 translate-y-4" data-reveal>
        <h2 className="mb-8 text-center text-3xl font-bold text-slate-900 md:mb-10 md:text-4xl">{heading}</h2>
      </div>
      <div
            ...
            <div key={`group-${groupIndex}`} className="flex shrink-0 gap-4 px-4 md:gap-6 md:px-6">
              {group.map((item, index) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5 md:gap-3">
                      <img
                        ...
                        alt={item.name}
                        loading="lazy"
                        className="h-10 w-10 rounded-full border border-slate-300 object-cover md:h-11 md:w-11"
                      />
                      <div>
                        <p className="text-sm font-bold tracking-tight text-slate-900 md:text-base">{item.name}</p>
                        <p className="text-xs text-slate-500">{item.location ?? location}</p>
                      </div>
                    </div>
                    <span className="inline-flex items-center rounded-full border border-slate-300 bg-white p-1">
                  ...
                  <p className="mt-3.5 text-sm leading-relaxed text-slate-600 antialiased md:mt-4 md:text-[15px]">“{item.quote}”</p>
